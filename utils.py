...
def get_list_of_groups():
    groups = MitreAttack.mitre_attack_data.get_groups(remove_revoked_deprecated=True)
    data =[]
    for g in groups:

        if hasattr(g, "aliases"):
                data.extend(g.aliases)
        data.append(g.name)
    return list(set(data))

def get_list_of_campaigns():
    campaigns = MitreAttack.mitre_attack_data.get_campaigns(remove_revoked_deprecated=True)
    data =[]
    for c in campaigns:

        if hasattr(c, "aliases"):
                data.extend(c.aliases)
        data.append(c.name)
    return list(set(data))
