...
huggingface_hub.login("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")
from datasets import load_dataset
campaigns_dir = Keys.CAMPAIGN_PATH
campaigns_output_dir = campaigns_dir + "/report_graphs"
campaigns_image_dir = campaigns_dir + "/images"
campaigns_input_dir = campaigns_dir + "/reports"
...
def graph_buidling_and_alignment(id:str = "", text:str = "", procedure_dict:dict = None, tech_dict:dict = None, procedure_phrases = None):
    big_campaign = BigCampaign(text, id)
    bert_similarity = CosineSimilarity()
    procedures_phrases = procedure_phrases
    campaign_phrases = big_campaign.phrases

    bert_similarity.compute_range_imprved(procedures_phrases,campaign_phrases )
    result = Alignment.all_alignment_sequential_big_campaign_sequential2(big_campaign, procedure_dict, tech_dict, bert_similarity)
    with open(os.path.join(campaigns_procedure_alignment_dir, id + ".json"), "w") as f:
        json.dump(result, f, indent=4)
    technique_alignment = Alignment.bigcampaign_technique_alignment(big_campaign, tech_dict)
    with open(os.path.join(campaigns_tech_alignment_dir, id + ".json"), "w") as f:
        json.dump(technique_alignment, f, indent=4)
        
class Manager():
    def __init__(self, campaign_from_0 = True, procedure_from_0 = True, technique_from_0 =True,techniue_alignment_from_0 =True, matching_from_0 = True, multiprocessing = False,context_similarity_from0 = False, do_procedure_deduplication = True) -> None:
        ...
        self.analyze_campaign_from_huggingface()
    ...
    def analyze_campaign_from_huggingface(self, path: str="kerinin/hackernews-stories"):
        ds_iterator = load_dataset(path, split='train')
        procedure_phrases = get_procedure_phrases(self.procedures)
        count = 0
        # tst_file = "data/CTI_report/reports/APT41 Campaign.txt"
        # with open(tst_file, "r") as f:
        #     text = f.read()
        #     campaign_id = "APT41 Campaign"
        #     graph_buidling_and_alignment(campaign_id, text, self.procedures, self.techniques, procedure_phrases )
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers= 2) as executor:
            for sample in ds_iterator:
                if count > 20:
                    break
                text = sample["Text"]
                campaign_id = sample["Title"]
                executor.submit(graph_buidling_and_alignment, campaign_id, text, self.procedures, self.techniques ,procedure_phrases)
                # graph_buidling_and_alignment(campaign_id, text, self.procedures, self.techniques, procedure_phrases )
                count += 1
    ...
    def big_procedure_matching(self):
        ...
        for campaign in self.big_campaigns:
            print("start analyzing this report "+campaign.id)
            result =   Alignment.all_alignment_sequential_big_campaign_sequential(campaign,self.procedures, self.techniques)
