...
def alignment_(campaign: Campaign, procedure:Procedure, technique):
        ...
        all_result = Alignment.graph_alignment(campaign,sub_graph, procedure)
        new_result = []
        for r in all_result:
            value, combination, edge_mapper = r
            alignment_rs = Alignment.generate_data_sample(combination, procedure, features, campaign.nodeID_2_order,campaign= campaign)
            alignment_rs["value"] = value
            alignment_rs["procedure_id"] = procedure.id
            alignment_rs["tech_id"] = procedure.special_id
            alignment_rs["edge_mapper"] = edge_mapper
            new_result.append(alignment_rs)
...
def alignment_with_range(campaign: Campaign, procedures: List[Procedure], technqiques: dict, bert_BERT_path:str = None, testing:bool = False):
    ...
    for procedure in procedures:
        ...
        if rs_ is None or len(rs_) == 0:
            continue
        rs.extend(rs_)

    return rs

def alignment_with_range2(campaign: Campaign, procedures: List[Procedure], technqiques: dict):
    rs = []
...
class Alignment():
    ...
    def graph_alignment(cls,campaign:Campaign, sub_graph:list , procedure: Procedure):
        ...
        for c in itertools.product(*v_list):
            ...
            if _sub_graph_similarity >= max_value:
                if _sub_graph_similarity > Keys.GLOBAL_THRESHOLD:
                    all_result.append((_sub_graph_similarity,_mapper,edge_alignment))
    ...
    @classmethod
    def generate_data_sample(cls, combination, procedure: Procedure, features: list = None, id2order:dict = None, campaign: Campaign = None):
        ...
        for k,v in combination.items():
            ...
            mapper.append(_data)
        sent_indexes = sorted(sent_indexes)
        min_sent = sent_indexes[0]
        max_sent = sent_indexes[-1]
        sent_text = []
        for i in range(min_sent, max_sent+1):
            sent_text.append(campaign.sentences[i].text)
        sent_text = " ".join(sent_text)
        procedure_text = []
        for i in range(len(procedure.sentences)):
            procedure_text.append(procedure.sentences[i].reverse_replacement(procedure.sentences[i].text)[0])
        procedure_text = " ".join(procedure_text)
        return_data = dict()
        return_data["sent_text"] = sent_text
        return_data["procedure_text"] = procedure_text
        return_data["nodes"] = good_nodes
        return_data["mapper"] = mapper
        return_data["sent_indexes"] = sent_indexes
        return_data["combine_ids"] = combine_ids
        return return_data
    @classmethod
    ...
    def all_alignment_sequential_big_campaign_sequential(cls, bigcampaign:BigCampaign, procedures: dict, techniques: dict):
        ...
        for campaign in bigcampaign.data:
            ...
            for i in range(0, max_numer_of_procedures, NUM_PROCEDURES_PER_PROCESS):
                ...
                rs = alignment_with_range(campaign, procedure_slice, techniques, bert_sim_path)
                final_result.extend(rs)
        return final_result         

    @classmethod
    def all_alignment_sequential_big_campaign_sequential2(cls, bigcampaign:BigCampaign, procedures: dict, techniques: dict, consine_similarity: CosineSimilarity):
        final_result = []
        global bert_similarity
        bert_similarity = consine_similarity
        for campaign in bigcampaign.data:
            keys = list(procedures.keys())
            max_numer_of_procedures = len(procedures)
            for i in range(0, max_numer_of_procedures, NUM_PROCEDURES_PER_PROCESS):
                ...
                procedure_id_slice = keys[start:end]
                procedure_slice = [procedures[k] for k in procedure_id_slice]
                rs = alignment_with_range2(campaign, procedure_slice, techniques)
                final_result.extend(rs)
        return final_result    
    ...
    def campaign_technique_alignment(cls, campaign:Campaign,  techniques: dict,id2order:dict):
        ...
        for k,v in techniques.items():
            ...
            for node in nodes:
                ...
                flag_, regconized_text =check_texts_similarity_simple(technique.best_phrases, texts)
                if flag_:    
                    node_id = id2order[id_]
                    if technique.id not in uniques:
                        temp_data = dict()
                        temp_data["techID"] = technique.id
                        temp_data["recognized_text"] = regconized_text
                        ...
                        sent_text = []
                        for i in range(len(campaign.sentences)):
                            sent_text.append(campaign.sentences[i].reverse_replacement(campaign.sentences[i].text)[0])
                        sent_text = " ".join(sent_text)
                        temp_data["sent_text"] = sent_text
